...
public class ProxyManager implements IProxyManager {
	...
	/**
	 * maps the partitions ID to the physical partition A partitions a @see
	 * {@link TreeMap} mapping objects and their node ID.
	 */
	...
	public boolean matchProxy(INeo4emfObject eObject,
			INeo4emfObject proxyEObject) {
		boolean result = object2proxy.containsKey(eObject);
		if (result)
			object2proxy.put(eObject, proxyEObject);
		return result;
	}
	...
	public INeo4emfObject getEObject(int partitionId, long nodeId) {
		return (partitions.containsKey(partitionId) ? partitions.get(
				partitionId).containsKey(nodeId) ? partitions.get(partitionId)
				.get(nodeId) : null : null);
	}

	public void addUsageTrace(int usedPID, int userPID, int featureId,
			EObject eObj) {
		if (!usageTraces.containsKey(usedPID))
			usageTraces.put(usedPID, new ArrayList<UsageTrace>());
	...
	public int getLeastFrequentlyPartition() {
		...
		for (int i = 0; i < partitionsUsageCount.size(); i++)
			if (minUsage.compareTo(partitionsUsageCount.get(i)) > 0
					&& partitions.containsKey(partitionsUsageCount.get(i).PID)) {
	...
	@Override
	public void updatePartitionsHistory(INeo4emfObject eObject, int feature,
			boolean isReference) {
		if (!isReference)
	...
	private int GetReferencePartitionID(INeo4emfObject eObject)
		...
		for (Entry<Integer, AbstractPartition> entry : partitions.entrySet()) {
			...
			switch (bol) {
			case 1:
				if (((INeo4emfObject) ((Partition) entry.getValue())
						.geteObjet()).getNodeId() == eObject.getNodeId())
					return entry.getKey();
				break;
			...
			case 0:
				Logger.log(
						IStatus.WARNING,
						"FLatPartition does not have an ID : "
								+ entry.getValue());
				return -1;
	...
	public void movePartitionTo(INeo4emfObject obj, int newIndex, int oldIndex) {
		// if (partitions.containsKey(oldIndex) &&
		// partitions.containsKey(newIndex)){
		...
		if (usageTraces.containsKey(key))
			for (UsageTrace trace : usageTraces.get(key)) {
				if (trace.eObject.equals(neoObj)) {
					...
					List<? extends INeo4emfObject> list = (partitions
							.get(trace.PID) instanceof Partition) ? ((Partition) partition)
							.getTailByFeatureID(trace.featureID)
	...
	public INeo4emfObject getObjectFromProxy(EClass eClassifier, Node n) {
		if (nodes2objects.containsKey(eClassifier)) {
			return (INeo4emfObject) (nodes2objects.get(eClassifier)
					.containsKey(n.getId()) ? nodes2objects.get(eClassifier)
					.get(n.getId()) : null);
		}
		return null;
	}
