...
public class Test {
	...
	public static void main(String[] args) throws Exception {
		...
		MEdge notInResourceEdge2 = factory.createMEdge();
		notInResourceEdge2.setName("notInResourceEdge2");
		node.getTo().add(notInResourceEdge2);
		
		MEdge inResourceAfterAddEdge = factory.createMEdge();
		inResourceAfterAddEdge.setName("inResourceAfterAddEdge");
		node.getTo().add(inResourceAfterAddEdge);
		
		graph.getEdges().add(inResourceAfterAddEdge);
		
		tmpSave(resource);
		/*
		 * Should throw exception like EMF (some referenced objects are not contained in
		 * a resource)
		 */
		resource.save(null);
	}
