...
public class KyanosURI extends URI {
	...
	public static final String KYANOS_HBASE_SCHEME = "kyanoshbase";
	
	protected URI internalUri;
	
	protected KyanosURI(int hashCode, URI internalUri) {
		super(hashCode);
	...
	public static URI createKyanosMapURI(File file) {
		URI fileUri = URI.createFileURI(file.getAbsolutePath());
		URI uri = URI.createHierarchicalURI(
				KyanosURI.KYANOS_MAP_SCHEME, 
				fileUri.authority(),
				fileUri.device(),
				fileUri.segments(),
				fileUri.query(),
